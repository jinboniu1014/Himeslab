...
GetACS <- function(vars_vec){
  ...
  acs_all <- get_acs(survey = "acs5", year = 2020, variables = vars_vec,state = "PA",geography = "block group",county = "Philadelphia")
...
shapes_phily <- tigris::block_groups(state = "PA",county = "Philadelphia",
                                     year = 2020)
...
adi_score_phily <- ggplot() +
  ...
  labs(title = "ADI Score in Philadelphia", 
       subtitle = "by Block Group Subdivision | ACS 2016-20") +
...
```{r}
ggsave("Map Visuals/ADI ACS/ADI ACS phily (block).png",adi_score_phily,width = 10, height = 8, dpi = 300)
```

```{r}
ggsave("Map Visuals/ADI ACS/ADI Percentile ACS phily (block).png",adi_percentile_phily,width = 10, height = 8, dpi = 300)
```

```{r}
ggsave("Map Visuals/ADI ACS/ADI raw ACS phily (block).png",adi_raw_phily,width = 10, height = 8, dpi = 300)
```
