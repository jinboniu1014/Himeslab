...
```{r}
#census_api_key("48b57731d07de058f688bf8e22a0ea11be5efe7f",install = TRUE)
```
...
GetACS <- function(vars_vec){
  ...
  acs_pa <- get_acs(name = "acs/acs5", year = 2020, variables = vars_vec,state = "PA",geography = "county")
  acs_nj <- get_acs(name = "acs/acs5", year = 2020, variables = vars_vec, state = "NJ",geography = "county")
  acs_de <- get_acs(name = "acs/acs5", year = 2020, variables = vars_vec, state = "DE",geography = "county")
  
  # Combine into a single data frame
  acs_all <- bind_rows(acs_pa, acs_nj, acs_de)
  #acs_all <- mutate(acs_all, GEOID = paste0(state, county, tract, block_group))
  return(acs_all)} 
```

## Get ACS Variables Needed to Compute ADI

```{r}
vars_education <- c("B15003_001E", "B15003_002E", "B15003_003E", "B15003_004E", 
...
```{r}
# 3b. Occupation... the following categories were considered white collar:
...
shapes <- tigris::counties(state = c("PA","NJ","DE"))

shapes_area <- shapes |>
  mutate(ALAND = st_area(shapes))|>
  mutate(A_KMSQ = ALAND/1,000,000) |>
  select(GEOID, A_KMSQ)

units(shapes_area$A_KMSQ) <- "km^2"

pop <- full_join(pop_raw, shapes_area, by = "GEOID") |>
...
acs_full <- full_join(edu, whc, by = "GEOID")|>
  full_join(inc, by = "GEOID")|>
  full_join(hou,by = "GEOID")|>
  full_join(ten, by = "GEOID")|>
  full_join(une, by = "GEOID")|>
  full_join(pov, by = "GEOID")|>
  full_join(spa, by = "GEOID")|>
  full_join(hqu, by = "GEOID")|>
  full_join(race, by = "GEOID")|>
  full_join(hhsize, by = "GEOID")|>
  full_join(pop, by = "GEOID")|>
  full_join(age,by = "GEOID") |>
...
acs_full$ADI_perc <- as.integer(NA)
acs_full$ADI_perc[! is.na(acs_full$ADI_score)] <- percentile(na.omit(acs_full$ADI_score))

# Edit and Save
acs_full <- filter(acs_full, !is.na(less9))
saveRDS(acs_full, "Map/ADI_tri_state.rds")
```

```{r}
mapacs <- left_join(acs_full, shapes, by="GEOID") |>
  st_as_sf()
...
```{r}
adi_score <- ggplot() +
  geom_sf(data = mapacs, aes(fill = ADI_score), lwd=0) +
  scale_fill_distiller(name = "ADI Score", palette = "YlOrRd", direction = 1) +
  labs(title = "ADI Score in PA,NJ,DE", 
       subtitle = "by County Subdivision | ACS 2016-20") +
  xlab("Area Deprivation Index is based on 17 poverty, \neducation, housing, and unemployment indicators.") +
  geom_sf(data = counties, color = "gray20", lwd=0.2, fill = NA) +
  plot_theme()
...
```{r}
adi_percentile <- ggplot() +
  geom_sf(data = mapacs, aes(fill = ADI_perc), lwd=0) +
  scale_fill_distiller(name = "ADI Percentile", palette = "YlOrRd", direction = 1) +
  labs(title = "ADI Percentile in in PA,NJ,DE", 
       subtitle = "by County Subdivision | ACS 2016-20") +
  xlab("Area Deprivation Index is based on 17 poverty, \neducation, housing, and unemployment indicators.") +
  geom_sf(data = counties, color = "gray20", lwd=0.2, fill = NA) +
  plot_theme()
print(adi_percentile)
```

```{r}
adi_raw <- ggplot() +
  geom_sf(data = mapacs, aes(fill = ADI_raw), lwd=0) +
  scale_fill_gradient2(name = "Raw ADI Score") +
  labs(title = "Raw ADI Score in PA,NJ,DE", 
       subtitle = "by County Subdivision | ACS 2016-20") +
  xlab("Area Deprivation Index is based on 17 poverty, \neducation, housing, and unemployment indicators.") +
  geom_sf(data = counties, color = "gray20", lwd=0.2, fill = NA) +
  plot_theme()
...
```{r}
ggsave("Map Visuals/ADI ACS/ADI ACS tri_state.png",adi_score,width = 10, height = 8, dpi = 300)
```

```{r}
ggsave("Map Visuals/ADI ACS/ADI Percentile ACS tri_state.png",adi_percentile,width = 10, height = 8, dpi = 300)
```

```{r}
ggsave("Map Visuals/ADI ACS/ADI raw ACS tri_state.png",adi_raw,width = 10, height = 8, dpi = 300)
```
