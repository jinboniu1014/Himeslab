...
```{r}
#census_api_key("111e7e690e9685c8fc99498d29641991101660b4",install = TRUE,overwrite = TRUE)
```
...
GetACS <- function(vars_vec){
  ...
  acs_all <-  get_acs(survey = "acs5", year = 2020, variables = vars_vec, geography = "county")|>
    filter(substr(GEOID, 1, 2) != "02" & substr(GEOID, 1, 2) != "15")
...
pop_nation <- full_join(pop_raw, shapes_area, by = "GEOID") |>
...
acs_full <- full_join(edu_nation, whc_nation, by = "GEOID")|>
  full_join(inc_nation, by = "GEOID")|>
  full_join(hou_nation,by = "GEOID")|>
  full_join(ten_nation, by = "GEOID")|>
  full_join(une_nation, by = "GEOID")|>
  full_join(pov_nation, by = "GEOID")|>
  full_join(spa_nation, by = "GEOID")|>
  full_join(hqu_nation, by = "GEOID")|>
  full_join(race_nation, by = "GEOID")|>
  full_join(hhsize_nation, by = "GEOID")|>
  full_join(pop_nation, by = "GEOID")|>
  full_join(age_nation,by = "GEOID") |>
...
adi_score <- ggplot() +
  ...
  labs(title = "ADI Score in the U.S.", 
  ...
  geom_sf(data = border, color = "gray20", lwd=0.2, fill = NA) +
  plot_theme()
...
adi_percentile <- ggplot() +
  ...
  labs(title = "ADI Percentile in in the U.S.", 
       subtitle = "by County Subdivision | ACS 2016-20") +
  xlab("Area Deprivation Index is based on 17 poverty, \neducation, housing, and unemployment indicators.") +
  geom_sf(data = border, color = "gray20", lwd=0.2, fill = NA) +
  plot_theme()
...
adi_raw <- ggplot() +
  ...
  labs(title = "Raw ADI Score in the U.S.", 
       subtitle = "by County Subdivision | ACS 2016-20") +
  xlab("Area Deprivation Index is based on 17 poverty, \neducation, housing, and unemployment indicators.") +
  geom_sf(data = border, color = "gray20", lwd=0.2, fill = NA) +
  plot_theme()
...
```{r}
ggsave("Map Visuals/ADI ACS/ADI ACS USA.png",adi_score,width = 10, height = 8, dpi = 300)
```

```{r}
ggsave("Map Visuals/ADI ACS/ADI Percentile ACS USA.png",adi_percentile,width = 10, height = 8, dpi = 300)
```

```{r}
ggsave("Map Visuals/ADI ACS/ADI raw ACS USA.png",adi_raw,width = 10, height = 8, dpi = 300)
```
